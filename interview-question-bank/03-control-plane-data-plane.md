...
第三组是 `PolicyService`，包括 `GetPolicy` 和 `WatchPolicy`。SDK 启动时先拉取策略快照，之后通过 watch 接收版本更新。
...
SDK policy manager 到 Controller：启动时 `GetPolicy`，运行中 `WatchPolicy` 接收策略版本更新。
...
Policy 更适合做 copy-on-write。每次加载 YAML 后生成不可变 snapshot，用版本号和原子指针发布；读路径只读当前 snapshot，不在热路径上解析文件。
...
Policy snapshot 应该是版本化的。写入时用 revision 或 CAS，SDK watch 时按 version 判断是否更新。这样多个 Controller 下发的是同一个版本语义。
...
可以把每个实例写成一个带 TTL 的 key，比如 `/aegis/registry/{service}/{instance}`，value 里放 address、metadata、version 和注册时间。

实例启动时创建 lease，心跳时续租。实例异常退出后 lease 过期，key 自动消失。Controller 或 SDK 可以 watch service 前缀，拿到新增、更新和删除事件。

health state 和 policy 不建议混在同一个 key 里。health 可以放 `/aegis/health/{service}/{instance}`，带较短 TTL 或版本；policy 可以放 `/aegis/policy/{service}`，使用 revision 做版本控制。

这样注册、健康和策略各自有清晰的生命周期。

### Q080【拓展】控制面配置强一致和最终一致分别适合哪些治理策略？

强一致适合影响安全边界和配置语义的内容，比如租户权限、认证策略、全局禁用开关、策略版本发布、服务注册租约。

最终一致适合观测驱动的动态数据，比如 slow_score、endpoint 权重、EWMA、Prometheus 指标和局部路由偏好。这些数据本来就来自采样，短时间不一致可以接受。

故障摘除介于两者之间。它不一定需要强一致，但需要版本、滞回和最小窗口，避免不同 Controller 或不同 SDK 在同一时间做出完全相反的路由决策。
...
策略隔离还需要 RBAC 和版本审计。不同团队只能改自己服务的策略，发布记录要能回滚。
...
第一步是服务发现，拿到候选实例。第二步是基础健康过滤，`DEAD` 和 `EJECTED` 不进入正常流量，`PROBING` 只保留探测资格。第三步应用灰度规则，比如按版本、用户标签或百分比得到目标权重。第四步把 slow_score、endpoint state 和网络分数叠加到权重上。最后由 SDK 的 adaptive P2C 根据本地 EWMA 和 in-flight 选具体 endpoint。

安全策略应该能压过灰度策略。比如灰度规则允许 10% 流量进 v2，但 v2 的某个 endpoint 被 `EJECTED`，那这个 endpoint 的正常流量应该是 0，只允许后续 PROBING。否则灰度发布会把故障治理抵消掉。

所有合并结果都应该带 policy version，trace 里也要记录 route version，方便 verifier 复盘实际流量是否符合预期。
