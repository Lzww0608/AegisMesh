...
SDK 的 adaptive balancer 在 Pick 阶段选出 endpoint 后，会调用：
...
也就是说，当某个 endpoint 的 in-flight 达到上限时，Pick 阶段直接失败，上层看到的是 `RESOURCE_EXHAUSTED`。
...
inflight breaker 是本地、即时的保护。它只看一个 endpoint 当前有多少请求还没完成。超过上限，就在 Pick 阶段快速失败，不再把新请求发出去。
...
所以一次请求如果在 Pick 阶段被 breaker 拒绝，retry interceptor 会看到 `RESOURCE_EXHAUSTED`，判断它不是默认可重试错误码，然后停止。
...
面试里我会说：这是项目阶段的简化实现。更细的版本应该让 breaker scope 可配置，比如：
