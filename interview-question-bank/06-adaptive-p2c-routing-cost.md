...
### Q148【简单】circuit breaker 在 Pick 阶段起什么作用？
...
Pick 阶段知道请求被发给了哪个 SubConn，也能记录开始时间。RPC 完成后，gRPC 会调用 `PickResult.Done`。这时 SDK 能算出本次 attempt 的耗时，也知道该把刚才增加的 inflight 减掉。
...
我更倾向于“两阶段”：先用 locality policy 过滤或分层，再用 adaptive P2C 处理同一层内部的负载和慢故障。这样策略更容易解释。否则 locality、slow_score、latency、weight 全混在一个公式里，调参会很难。

### Q164【拓展】如果要支持 weighted canary，weight、slow_score、version policy 的优先级如何定义？

我会把优先级拆清楚。

第一层是安全过滤。`DEAD/EJECTED` 不接正常流量，最多等进入 `PROBING` 后接探测流量。这个优先级最高。

第二层是 version policy。比如 v2 只接 5% canary 流量，v1 接 95%。这个规则决定每个版本的基础流量份额。

第三层是版本内部的 endpoint weight。比如同为 v2，有的实例权重大，有的权重小。

第四层是 slow_score 和本地 cost。它在版本份额内进一步调整，避开慢 endpoint。

例子是：policy 允许 v2 接 5%，但 v2 某个 endpoint 被 `EJECTED`，那它的正常流量应该是 0。如果 v2 有多个健康 endpoint，5% 流量再按 weight 和 adaptive P2C 分配。如果 v2 只剩 PROBING endpoint，是否继续 canary 要看策略，通常应该暂停正常 canary，只做探测。
