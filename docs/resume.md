...
- Built EWMA latency windows, relative + absolute SLO slow_score, endpoint state machine with PROBING probe ratio, adaptive P2C routing, retry budgets, and endpoint circuit breaker. Slow endpoints are down-weighted first, then ejected, probed, and returned after recovery.
...
- **Fail-slow detection**: not just health checks; endpoint telemetry includes EWMA, p95, error rate, in-flight pressure, and optional TCP retransmit/connect signals.
...
- **Trace verification**: SDK emits real JSONL traces with route metadata; verifier checks route distribution, retry attempts, and forbidden service edges.
- **Project packaging**: Docker Compose, Makefile, CI, experiment scripts, result checker, report docs, Prometheus config, Grafana dashboard, and reproducibility tests are included.
...
For newer mechanisms, mention the two validation runs. PROBING traffic stayed at 0.2177% of user-service trace rows, so recovery traffic was limited. Absolute SLO scoring raised max slow_score from 0.377 to 1.007 in an all-slow control case, which shows why peer comparison alone is not enough.
