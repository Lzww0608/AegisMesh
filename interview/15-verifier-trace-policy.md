...
第一类是 route distribution，也就是流量是否按预期打到不同 route 上。比如 canary 场景里，希望 `user-service:primary` 承接 90%，`user-service:secondary` 承接 10%。verifier 会统计 trace 里的 `route` 字段，再和 spec 里的期望比例比较。
...
举个例子，期望 `user-service:secondary` 是 10%，tolerance 是 3%，那么实际比例在 7% 到 13% 之间就算通过。
...
这个检查适合发现服务绕过网关、绕过中间层、错误调用新发布变体、或者策略配置把流量导到了不该去的下游。
...
```text
期望 secondary 10%，实际 secondary 10.8%，tolerance=3%，route_distribution check passed。
```
...
举个例子，100 条 trace 里有 90 条 route 是 `primary`，10 条缺失 route。当前算法算出来是：
...
- 同一个服务有 revision、zone、instance 等多个维度；
- path 里有空格或多余前后缀。
...
再根据 spec 决定 forbidden edge 是按 service 粒度、revision 粒度，还是 instance 粒度匹配。当前项目为了本地实验可读性，直接用字符串；这是可接受的 MVP，但不是长期最稳的表示方式。
...
最简单的估计可以用二项分布。比如金丝雀期望 secondary 是 10%，采样到 n 条 trace，实际 secondary 比例是 `p_hat`。标准误差大致是：
...
好处是 schema 可以向前演进。今天 verifier 只校验 route、retry 和 forbidden edge；明天可以加 method 级策略、时间窗口、PROBING ratio、route revision、policy revision，而不需要改 SDK 的日志格式。
...
- 两条都走 `user-service:primary`，但 spec 期望 primary/secondary 各 50%；
- 第一条 path 是 `frontend -> payment-service`，命中 forbidden edge；
- 第一条 `retry_attempts=2`，超过 `max_retry_attempts=1`。

这条测试会同时触发三个失败：

- `route_distribution:user-service:primary`
- `retry_budget`
- `forbidden_edges`
...
```text
route("user-service:secondary").ratio between 0.07 and 0.13
...
第一步是离线策略检查。策略 PR 提交后，CI 用 sample trace 和 negative trace 跑 verifier，保证规则语法、边界和失败路径没问题。

第二步是 staging replay。把候选 policy 部署到 staging，跑一组固定流量，收集真实 JSONL trace，再执行 verifier。比如：
...
第三步是 canary 后校验。新策略只对小流量生效，持续收集 trace。只有 verifier 连续几个窗口通过，才扩大流量。
...
简单做法是二项分布近似。假设 secondary 期望比例是 10%，观察到 n 条 trace，其中 k 条走 secondary：
