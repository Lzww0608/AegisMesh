...
`service` 是策略所属服务，比如 `user-service`。`revision` 是这份快照的修订，用来判断策略有没有变化。
...
每个 service 都会生成一份 `PolicySnapshot`。它的 `Revision` 用 policy 文件的 `ModTime().UnixNano()`。
...
`WatchPolicy` 是流式订阅。SDK 和 Controller 建立一个 stream，Controller 发现策略 revision 变化后，把新的 `PolicySnapshot` 推给 SDK。
...
### Q344【简单】policy revision 使用文件 modTime 有什么作用？
...
然后写入 `PolicySnapshot.Revision`。

Controller 的 `WatchPolicy` 维护一个 `lastRevision`。如果当前 snapshot 的 revision 和上次一样，就不发送；如果变了，就推给 SDK：
...
SDK 侧也用 revision 管理 retry budget。`dynamicRetrySource` 会按 method 和 policy revision 维护 budget。如果 revision 变了，它会为这个 method 重建预算。

所以 revision 至少有两个作用：减少重复推送，让 SDK 知道策略已经变更。
...
### Q347【深度】用文件 modTime 作为 revision 是否足够可靠？有哪些边界情况？

本地实验够用，生产不够。

modTime 最大的优点是简单。文件改了，时间变了，revision 也变了。对单机 demo、Docker Compose 实验、手动改 YAML 来说，这个方案很方便。

边界也不少。

第一，文件系统时间精度不一定稳定。有些环境 modTime 精度比较粗，短时间连续写多次，可能看起来是同一个时间。

第二，内容没变但 modTime 变了，也会触发新 revision。比如只 touch 文件，策略内容没变，SDK 仍然可能收到新快照，retry budget 也可能重建。

第三，内容变了但 modTime 没变，也可能被漏掉。比如某些部署工具保留时间戳，或者复制文件时带了原时间。

第四，多 Controller 场景里，不同机器的文件时间和时钟不一定一致。用本地 modTime 做全局修订，很难保证顺序。

更稳的做法是使用显式修订号、配置中心 revision、etcd revision，或者对内容做 hash。生产里我会让每次发布生成一个单调递增的 policy revision，rollback 也生成新 revision，而不是依赖文件时间。
...
### Q351【深度】如果 WatchPolicy 收到旧修订快照，是否应该拒绝？

一般应该拒绝，至少要识别出来。

当前 SDK 的 `policyManager.Update()` 不检查 revision。它拿到 snapshot 就覆盖：
...
但多 Controller 或重连场景会复杂。SDK 可能先连到一个新 Controller，收到 revision 100；后来重连到旧 Controller，又收到 revision 90。如果直接覆盖，客户端就回到了旧策略。

理想做法是：

- 如果 revision 单调递增，SDK 拒绝比当前 revision 小的快照。
- 如果要回滚，也不要下发旧 revision，而是生成一个新的 revision，内容回到旧策略。
- 对相同 revision、不同内容的快照直接报警，这是严重发布问题。

所以我的回答是：当前代码还没有做修订单调检查；生产环境应该补。回滚不是“发旧修订”，而是“用新修订发布旧内容”。

### Q352【深度】Retry budget 按 policy revision 重建会带来什么副作用？
...
key 是 method。如果某个 method 的 budget 不存在，或者 snapshot revision 变了，就创建新的 budget：
...
坏处是频繁 policy 更新可能绕开预算。比如故障期间每 3 秒更新一次 revision，每次都重建 budget，每次都有新的 minBudget。这样重试总量会比预期更高。
...
- 兼容字段变更时迁移已有计数，而不是清零。
- 控制面限制发布频率。
- 将 budget key 从 policy revision 改成 retry config hash，routing 或 outlier 变化不影响 retry budget。
...
SDK 侧，每个 ClientConn 都有 watcher goroutine。更新频繁时，policyManager 会频繁加锁更新。retry source 看到 revision 变化后，还可能频繁重建 budget。
...
- 给 policy watcher 暴露 metrics，比如当前修订、更新次数、失败次数、stale 时间。
...
回滚：每次发布都应该能一键回到上一个稳定修订。更准确地说，是用一个新 revision 发布旧内容，而不是让客户端回退到旧 revision。
...
观测：要知道每个客户端当前拿到哪个 policy revision，更新是否成功，失败多久了。

AegisMesh 当前实现的是最小可用闭环：YAML -> PolicyService -> SDK watcher。它已经能说明动态策略闭环，但离平台级配置系统还差灰度、权限、审计和回滚。
...
推送响应快，但要处理连接状态、重连、消息顺序和修订去重。客户端断线期间错过了更新，重连后必须拿到最新全量快照，而不是只等下一次增量。
...
- 客户端之间收到新策略的时间不同。
- stream 断线期间客户端使用旧策略。
- 多 Controller 场景可能发送不同修订。
- 旧 revision 可能覆盖新 revision。

解决办法还是 revision。每个 snapshot 要有单调修订，客户端只接受更新修订；重连后先拿当前全量快照；控制面要能回答“哪些客户端已经生效到哪个修订”。
...
这个方向能把项目从“手动实验”推进到“策略闭环评估”。AegisMesh 现在已经有实验脚本、CSV checker、verifier 和 dashboard，后续可以把 policy revision 接进这些结果里。

### Q364【拓展】如果 policy 更新导致事故，如何快速回滚并证明影响范围？

先回滚，再复盘。

回滚时不要让客户端回到旧 revision，而是发布一个新 revision，内容恢复到上一个稳定策略。这样 SDK 的 revision 判断不会混乱，WatchPolicy 也能正常推送。
...
第一，配置审计：哪个 revision 在什么时间发布，改了哪些字段。

第二，客户端生效情况：哪些 source、哪些服务、哪些实例收到了这个 revision。当前项目还没有完整的 policy revision metric，这是可以补的。
...
当前 AegisMesh 可以用日志、Prometheus、实验 CSV 和 trace JSONL 做一部分证明。生产化后，我会补 `aegis_policy_revision{source,service}` 这类指标，以及策略审计日志。
